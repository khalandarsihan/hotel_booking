// frontend/src/components/ui/modals/BookingModal.jsx
import React, { useState, useEffect } from 'react';
import { X, User, Mail, Phone, Calendar, Building, Home, Bed, Users, Info, CreditCard, CheckCircle } from 'lucide-react';

const BookingModal = ({ isOpen, onClose, booking = null, onSubmit }) => {
  ...
  // Load properties and initialize form on component mount
  useEffect(() => {
    setProperties(mockProperties);
    
    // Initialize form with default values or booking data for edit mode
    if (booking) {
      setIsEditMode(true);
      setFormData({
          ...
          {!success && (
            <div className="mb-4">
              ...
              <div className="flex justify-between mt-2 text-xs">
                ...
                <span className={step >= 3 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}>Confirmation</span>
              </div>
            </div>
          )}
          ...
          {success && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Booking ID: <span className="font-medium">{isEditMode ? booking?.id : `B${Math.floor(1000 + Math.random() * 9000)}`}</span>
              </p>
            </div>
          )}
